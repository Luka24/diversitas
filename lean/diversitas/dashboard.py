...
COL_BLUE     = "#2962ff"   # blue       — allocation / info lines
COL_MA50     = "#2196f3"   # lighter blue  — 50 MA (trend)
...
def _build_price_chart(df: pd.DataFrame, symbol: str) -> go.Figure:
    ...
    # Trackline — coloured by slope filter (track_rising_window)
    ...
    # 50 MA — lighter blue (trend filter)
    fig.add_trace(go.Scatter(
        x=df.index, y=df["ma_med"],
        mode="lines", line=dict(color=COL_MA50, width=1.2),
        name="50 MA (trend)",
        hovertemplate="50 MA %{y:,.2f}<extra></extra>",
    ), row=1, col=1)

    # 200 MA — coloured by direction (orange=rising, red=falling)
    rising_long = (df["ma_long"] > df["ma_long"].shift(5)).fillna(False).to_numpy()
    ml = df["ma_long"].to_numpy()
    seg_start2, first_ml = 0, True
    for i in range(1, len(df) + 1):
        if i == len(df) or rising_long[i] != rising_long[seg_start2]:
            col = COL_MA200 if rising_long[seg_start2] else COL_BEAR
            end = i + (0 if i == len(df) else 1)
            fig.add_trace(go.Scatter(
                x=xs[seg_start2:end], y=ml[seg_start2:end],
                mode="lines", line=dict(color=col, width=1.2, dash="dot"),
                name="200 MA (regime)", showlegend=first_ml, legendgroup="ma200",
    ...
    # Allocation subplot — step line (binary 0/100)
    fig.add_trace(go.Scatter(
        x=df.index, y=df["target_alloc"],
        mode="lines", line=dict(color=COL_BLUE, width=1.8, shape="hv"),
...
def _build_vol_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color=COL_NEUTRAL, width=1.5),
        name="Annual vol %",
    ...
    if "vol_avg50" in win.columns:
        fig.add_trace(go.Scatter(
            ...
            name="Vol-shock threshold (1.5 × avg)",
...
def _build_trade_ledger(df: pd.DataFrame) -> list[dict]:
    ...
    for ts, row in changes.iterrows():
        ...
        if sig == S_BULL:
            ...
            exit_reason = ("blow-off" if bool(row["blowoff"])
            ...
            trades.append({
                **open_entry,
                ...
                "pnl_pct": pnl, "exit_reason": exit_reason, "open": False,
            })
            open_entry = None
    last = df.iloc[-1]
...
def main() -> None:
    ...
    with left:
        st.markdown(
            f'<div style="color:{COL_DIM};font-size:10px;'
            f'text-transform:uppercase;letter-spacing:1.5px;'
            f'margin:0 0 7px 0">Entry gates · all must PASS for BULL</div>',
            unsafe_allow_html=True,
        )
        last = df.iloc[-1]
        gates = [
    ...
    with right:
        st.markdown(
            f'<div style="color:{COL_DIM};font-size:10px;'
            f'text-transform:uppercase;letter-spacing:1.5px;'
            f'margin:0 0 7px 0">Status detail</div>',
            unsafe_allow_html=True,
        )
        ma_colour = (COL_BEAR if s["bear_regime"]
                     else COL_NEUTRAL if s["ma_long_status"] == "BELOW"
                     else COL_BULL)
        ...
        detail_rows = [
            _row("200 MA (regime)",  s["ma_long_status"],  ma_colour),
            _row("50 MA (trend)",
                 "ABOVE" if s["above_ma_med"] else "BELOW",
                 COL_BULL if s["above_ma_med"] else COL_BEAR),
            _row("Trackline slope",  tl_dir,               tl_colour),
            _row("RSI",              f"{rsi_val:.1f}" if not pd.isna(rsi_val) else "—",
                 rsi_col),
